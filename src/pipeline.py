...
from clip_scanner import scan_folder, summarize_folder
...
def run(config: Config) -> None:
    ...
    for i, (folder, (count, dur)) in enumerate(zip(char_folders, summaries), 1):
        info = f"{count} clips, {_fmt_duration(dur)}" if count else "no clips"
        print(f"  [{i}] {folder.name:<12}  {info}")
        logging.debug("Menu item %d: %s — %s", i, folder.name, info)
