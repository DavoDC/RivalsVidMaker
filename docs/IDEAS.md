...
**T1. Force-rescan all clips to rebuild cache with new algorithm data**

Now that pass 2 is 8fps + no early exit, all existing cache entries were generated by the old algorithm (4fps, early exit applied to both passes). Re-scan all clips to:
- Get accurate `scan_pass` data (which clips needed pass 2, proportion pass 1 vs pass 2)
- Verify previously-null single-KO clips (THOR_2026-03-17_22-20-29, THOR_2026-03-22_23-19-10, THOR_2026-03-27_22-23-58, THOR_2026-03-28_23-22-42) now detect the KO
- Update timing data for the scan-time estimation model (pass 2 at 8fps is slower)

How: use the force_rescan flag/option in the pipeline, or delete cache files and re-run `ko_detect.py --batch`.
